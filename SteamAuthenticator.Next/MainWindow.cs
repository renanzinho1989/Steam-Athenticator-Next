...
public class MainWindow : Window, IComponentConnector, IStyleConnector
{
	...
	private static readonly Dictionary<string, string> PtToEnStaticUiText = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
	{
		...
		{ "Aprove ou recuse trocas e confirmacoes do Mercado Steam.", "Approve or deny trades and Steam Market confirmations." },
		{ "Aprove ou recuse trocas e confirmações do Mercado Steam.", "Approve or deny trades and Steam Market confirmations." },
		{ "Todas", "All" },
		{ "Trocas", "Trades" },
		{ "Mercado", "Market" },
		{ "Pendentes", "Pending" },
		{ "Nada pendente para confirmar.", "Nothing pending to confirm." },
		{ "Quando houver uma nova confirmacao da Steam, ela vai aparecer aqui.", "When a new Steam confirmation appears, it will show up here." },
		{ "Quando houver uma nova confirmação da Steam, ela vai aparecer aqui.", "When a new Steam confirmation appears, it will show up here." },
		{ "Resumo", "Summary" },
		{ "pendentes no total", "pending total" },
		{ "trocas pendentes", "pending trades" },
		{ "itens de mercado", "market items" },
		{ "Dicas rápidas", "Quick tips" },
		{ "As confirmacoes expiram rapidamente.", "Confirmations expire quickly." },
		{ "As confirmações expiram rapidamente.", "Confirmations expire quickly." },
		{ "Aprove ou recuse o quanto antes para nao perder o prazo.", "Approve or deny as soon as possible to avoid missing the window." },
		{ "Aprove ou recuse o quanto antes para não perder o prazo.", "Approve or deny as soon as possible to avoid missing the window." },
		{ "Aprove ou recuse o quanto antes para não perder", "Approve or deny as soon as possible" },
		{ "Aprove ou recuse o quanto antes para nao perder", "Approve or deny as soon as possible" },
		{ "Mantenha o cofre protegido com uma senha forte.", "Keep your vault protected with a strong password." },
		{ "Mantenha o cofre protegido com uma senha forte", "Keep your vault protected with a strong password" },
		{ "Mantenha o cofre protegido com uma senh", "Keep your vault protected" },
		{ "Conta em foco", "Account in focus" },
		{ "Atualizar", "Refresh" },
		...
		{ "Fecha a janela e deixa o app ativo discretamente na bandeja do sistema.", "Closes the window and keeps the app quietly active in the system tray." },
		{ "Espaco reservado para inicializacao automatica em uma proxima etapa do app.", "Reserved space for automatic startup in a future app step." },
		{ "Espaço reservado para inicialização automática em uma próxima etapa do app.", "Reserved space for automatic startup in a future app step." },
		{ "Altere a aparencia geral mantendo o contraste da interface.", "Change the overall appearance while keeping interface contrast." },
		{ "Altere a aparência geral mantendo o contraste da interface.", "Change the overall appearance while keeping interface contrast." },
		{ "Escolha o idioma principal exibido em menus, mensagens e telas.", "Choose the main language shown in menus, messages, and screens." },
		{ "Indicador visual para o fluxo de protecao do cofre.", "Visual indicator for the vault protection flow." },
		{ "Indicador visual para o fluxo de proteção do cofre.", "Visual indicator for the vault protection flow." },
		{ "Expande a busca para todas as contas carregadas no cofre.", "Expands the search to all accounts loaded in the vault." },
	...
	private static readonly Dictionary<string, string> EnToPtStaticUiText = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
	{
		...
		{ "Approve or deny trades and Steam Market confirmations.", "Aprove ou recuse trocas e confirmacoes do Mercado Steam." },
		{ "All", "Todas" },
		{ "Trades", "Trocas" },
		{ "Market", "Mercado" },
		{ "Pending", "Pendentes" },
		{ "Nothing pending to confirm.", "Nada pendente para confirmar." },
		{ "When a new Steam confirmation appears, it will show up here.", "Quando houver uma nova confirmacao da Steam, ela vai aparecer aqui." },
		{ "Summary", "Resumo" },
		{ "pending total", "pendentes no total" },
		{ "pending trades", "trocas pendentes" },
		{ "market items", "itens de mercado" },
		{ "Confirmations expire quickly.", "As confirmacoes expiram rapidamente." },
		{ "Approve or deny as soon as possible to avoid missing the window.", "Aprove ou recuse o quanto antes para nao perder o prazo." },
		{ "Approve or deny as soon as possible", "Aprove ou recuse o quanto antes" },
		{ "Keep your vault protected", "Mantenha o cofre protegido" },
		{ "Account in focus", "Conta em foco" },
		{ "Refresh", "Atualizar" },
		...
		{ "Closes the window and keeps the app quietly active in the system tray.", "Fecha a janela e deixa o app ativo discretamente na bandeja do sistema." },
		{ "Reserved space for automatic startup in a future app step.", "Espaco reservado para inicializacao automatica em uma proxima etapa do app." },
		{ "Change the overall appearance while keeping interface contrast.", "Altere a aparencia geral mantendo o contraste da interface." },
		{ "Choose the main language shown in menus, messages, and screens.", "Escolha o idioma principal exibido em menus, mensagens e telas." },
		{ "Visual indicator for the vault protection flow.", "Indicador visual para o fluxo de protecao do cofre." },
		{ "Expands the search to all accounts loaded in the vault.", "Expande a busca para todas as contas carregadas no cofre." },
		...
		try
		{
			...
			foreach (SteamGuardAccount account in list)
			{
				if (await EnsureSessionForConfirmationsAsync(account))
				{
					...
					{
						IReadOnlyList<string> detailLines = await FetchConfirmationDetailLinesAsync(account, confirmation);
						_embeddedConfirmations.Add(new ConfirmationItem(confirmation, account, detailLines, !IsEnglishLanguageSelected()));
					}
				}
			}
	...
	private async Task<IReadOnlyList<string>> FetchConfirmationDetailLinesAsync(SteamGuardAccount account, Confirmation confirmation)
	{
		try
		{
			List<string> names = new List<string>();
			string detailsPageUrl = "https://steamcommunity.com/mobileconf/detailspage/" + confirmation.ID + "?" + account.GenerateConfirmationQueryParams("details");
			string detailsPageResponse = await SteamWeb.GETRequest(detailsPageUrl, account.Session.GetCookies());
			string tradeOfferId = ExtractTradeOfferId(detailsPageResponse);
			if (string.IsNullOrWhiteSpace(tradeOfferId) && confirmation.ConfType == Confirmation.EMobileConfirmationType.Trade && confirmation.Creator != 0UL)
			{
				tradeOfferId = confirmation.Creator.ToString();
			}
			if (!string.IsNullOrWhiteSpace(tradeOfferId))
			{
				string steamLanguage = IsEnglishLanguageSelected() ? "english" : "brazilian";
				string tradeOfferApiUrl = "https://api.steampowered.com/IEconService/GetTradeOffer/v1/?access_token=" + WebUtility.UrlEncode(account.Session.AccessToken) + "&tradeofferid=" + WebUtility.UrlEncode(tradeOfferId) + "&language=" + steamLanguage + "&get_descriptions=1";
				string tradeOfferJson = await SteamWeb.GETRequest(tradeOfferApiUrl, account.Session.GetCookies());
				AddTradeOfferItemNames(tradeOfferJson, names, !IsEnglishLanguageSelected());
				string tradeOfferUrl = "https://steamcommunity.com/tradeoffer/" + tradeOfferId + "/?l=" + steamLanguage;
				string tradeOfferHtml = await SteamWeb.GETRequest(tradeOfferUrl, account.Session.GetCookies());
				AddConfirmationItemNames(tradeOfferHtml, names, !IsEnglishLanguageSelected());
			}
			AddConfirmationItemNames(detailsPageResponse, names, !IsEnglishLanguageSelected());
			if (names.Count == 0)
			{
				string detailsUrl = "https://steamcommunity.com/mobileconf/details/" + confirmation.ID + "?" + account.GenerateConfirmationQueryParams("details");
				string detailsResponse = await SteamWeb.GETRequest(detailsUrl, account.Session.GetCookies());
				AddConfirmationItemNames(detailsResponse, names, !IsEnglishLanguageSelected());
			}
			AddMarketItemNameFromHeadline(confirmation.Headline, names);
			return IsEnglishLanguageSelected() ? names.ToList() : BuildPortugueseItemList(names);
		}
		catch
		{
	...
	private static string ExtractTradeOfferId(string response)
	{
		...
		try
		{
			...
			string id = obj.SelectToken("tradeoffer.id")?.ToString() ?? obj.Value<string>("tradeofferid") ?? string.Empty;
		...
		Match match = Regex.Match(response, "tradeofferid[_/](?<id>\\d+)|tradeoffer/(?<id>\\d+)|\"id\"\\s*:\\s*\"(?<id>\\d+)\"", RegexOptions.IgnoreCase);
		return match.Success ? match.Groups["id"].Value : string.Empty;
	}

	private static void AddConfirmationItemNames(string response, List<string> names, bool preferPortuguese)
	{
		if (string.IsNullOrWhiteSpace(response))
		{
			return;
		}
		string html = ExtractHtmlFromJson(response);
		AddItemNamesFromJsonDescriptions(response, names, preferPortuguese);
		AddItemNamesFromJsonDescriptions(html, names, preferPortuguese);
		AddItemNamesFromHtmlAttributes(html, names, preferPortuguese);
		AddItemNamesFromHtmlBlocks(html, names, preferPortuguese);
	}

	private static void AddTradeOfferItemNames(string response, List<string> names, bool preferPortuguese)
	{
		if (string.IsNullOrWhiteSpace(response))
		{
			return;
		...
		{
			JObject obj = JObject.Parse(response);
			JToken? offer = obj.SelectToken("response.offer");
			JToken? descriptions = obj.SelectToken("response.descriptions");
			if (offer == null || descriptions == null)
			{
				return;
			}
			Dictionary<string, string> descriptionNames = descriptions
				.OfType<JObject>()
				.Select((JObject description) => new
				{
					Key = BuildDescriptionKey(description),
					Name = BuildTradeOfferItemName(description, preferPortuguese)
				})
				.Where(x => !string.IsNullOrWhiteSpace(x.Key) && !string.IsNullOrWhiteSpace(x.Name))
				.GroupBy(x => x.Key, StringComparer.OrdinalIgnoreCase)
				.ToDictionary(g => g.Key, g => g.First().Name, StringComparer.OrdinalIgnoreCase);
			AddTradeOfferAssetNames(offer["items_to_give"], descriptionNames, names);
			AddTradeOfferAssetNames(offer["items_to_receive"], descriptionNames, names);
		}
		catch
		{
		}
	...
	private static string BuildTradeOfferItemName(JObject description, bool preferPortuguese)
	{
		string marketName = description.Value<string>("market_name") ?? string.Empty;
		string marketHashName = description.Value<string>("market_hash_name") ?? string.Empty;
		string name = description.Value<string>("name") ?? string.Empty;
		string selected = preferPortuguese
			? FirstNonEmpty(marketName, name, marketHashName)
			: FirstNonEmpty(marketHashName, marketName, name);
		return preferPortuguese ? TranslateSteamItemNameToPortuguese(selected) : selected;
	}
	...
	private static string TranslateSteamItemNameToPortuguese(string name)
	{
		...
		{
			translated = Regex.Replace(translated, Regex.Escape(replacement.Key), replacement.Value, RegexOptions.IgnoreCase);
		}
		...
		{
			translated = Regex.Replace(translated, "Kilowatt\\s+Case", "Caixa Kilowatt", RegexOptions.IgnoreCase);
		}
		return translated;
	}

	private static IReadOnlyList<string> BuildPortugueseItemList(IEnumerable<string> names)
	{
		List<string> translatedNames = new List<string>();
		foreach (string name in names)
		{
			string translated = TranslateSteamItemNameToPortuguese(name);
			if (string.IsNullOrWhiteSpace(translated))
			{
				continue;
			}
			int sameIndex = translatedNames.FindIndex((string x) => string.Equals(x, translated, StringComparison.OrdinalIgnoreCase));
			if (sameIndex >= 0)
			{
				continue;
			}
			int shorterIndex = translatedNames.FindIndex((string x) => translated.StartsWith(x + " (", StringComparison.OrdinalIgnoreCase));
			if (shorterIndex >= 0)
			{
				translatedNames[shorterIndex] = translated;
				continue;
			}
			if (translatedNames.Any((string x) => x.StartsWith(translated + " (", StringComparison.OrdinalIgnoreCase)))
			{
				continue;
			}
	...
	private static void AddTradeOfferAssetNames(JToken? assets, Dictionary<string, string> descriptionNames, List<string> names)
	{
		if (assets == null)
		{
			return;
		...
		{
			string key = BuildDescriptionKey(asset);
			if (descriptionNames.TryGetValue(key, out string name))
			{
				AddItemName(names, name);
			}
		}
	}

	private static string BuildDescriptionKey(JObject item)
	{
		string appId = item.Value<string>("appid") ?? string.Empty;
		string classId = item.Value<string>("classid") ?? string.Empty;
		string instanceId = item.Value<string>("instanceid") ?? "0";
		return string.IsNullOrWhiteSpace(appId) || string.IsNullOrWhiteSpace(classId) ? string.Empty : appId + ":" + classId + ":" + instanceId;
	}

	private static string ExtractHtmlFromJson(string response)
	{
		try
		{
			JObject obj = JObject.Parse(response);
			return obj.Value<string>("html") ?? response;
		}
		catch
		{
			return response;
	...
	private static void AddItemNamesFromHtmlAttributes(string html, List<string> names, bool preferPortuguese)
	{
		foreach (Match tagMatch in Regex.Matches(html, "<[^>]*(?:economy/image|trade_item|market_listing_item|item)[^>]*>", RegexOptions.IgnoreCase))
		{
			Match valueMatch = Regex.Match(tagMatch.Value, "(?:alt|title)\\s*=\\s*[\"'](?<value>[^\"']+)[\"']", RegexOptions.IgnoreCase);
			if (valueMatch.Success)
			{
				AddItemName(names, preferPortuguese ? TranslateSteamItemNameToPortuguese(valueMatch.Groups["value"].Value) : valueMatch.Groups["value"].Value);
			}
		}
	}

	private static void AddItemNamesFromJsonDescriptions(string text, List<string> names, bool preferPortuguese)
	{
		foreach (Match match in Regex.Matches(text, "\"(?:market_hash_name|market_name)\"\\s*:\\s*\"(?<value>(?:\\\\.|[^\"])*)\"", RegexOptions.IgnoreCase))
		{
			string value = Regex.Unescape(match.Groups["value"].Value);
			AddItemName(names, preferPortuguese ? TranslateSteamItemNameToPortuguese(value) : value);
		}
	}

	private static void AddItemNamesFromHtmlBlocks(string html, List<string> names, bool preferPortuguese)
	{
		foreach (Match match in Regex.Matches(html, "<[^>]*class\\s*=\\s*[\"'][^\"']*(?:market_listing_item_name|trade_item_name|trade_item_desc|item_name)[^\"']*[\"'][^>]*>(?<value>.*?)</[^>]+>", RegexOptions.IgnoreCase | RegexOptions.Singleline))
		{
			string value = StripHtml(match.Groups["value"].Value);
			AddItemName(names, preferPortuguese ? TranslateSteamItemNameToPortuguese(value) : value);
		}
	}

	private static void AddMarketItemNameFromHeadline(string? headline, List<string> names)
	{
		if (string.IsNullOrWhiteSpace(headline))
		{
			return;
		}
		Match match = Regex.Match(headline, "^(?:Sell|Vender|List|Listing)\\s*-\\s*(?<value>.+)$", RegexOptions.IgnoreCase);
		if (match.Success)
		{
			AddItemName(names, match.Groups["value"].Value);
		}
	}

	private static void AddItemName(List<string> names, string value)
	{
		string name = WebUtility.HtmlDecode(value ?? string.Empty).Trim();
		name = Regex.Replace(name, "\\s+", " ");
		if (!IsLikelyConfirmationItemName(name))
		{
			return;
		}
	...
	private static bool IsLikelyConfirmationItemName(string name)
	{
		if (string.IsNullOrWhiteSpace(name) || name.StartsWith("http", StringComparison.OrdinalIgnoreCase) || name.Contains("steamcommunity.com", StringComparison.OrdinalIgnoreCase))
		{
			return false;
		}
		string lower = name.ToLowerInvariant();
		if (lower is "steam" or "counter-strike 2" or "counter-strike" or "dota 2" or "blank" or "attribute")
		{
			return false;
		}
		if (lower.StartsWith("attr:", StringComparison.OrdinalIgnoreCase) || lower.StartsWith("classinfo", StringComparison.OrdinalIgnoreCase) || lower.StartsWith("rgdescriptions", StringComparison.OrdinalIgnoreCase))
		{
			return false;
		}
		if (name.Length < 3 || name.All((char c) => char.IsDigit(c) || char.IsPunctuation(c) || char.IsWhiteSpace(c)))
		{
			return false;
		}
		return true;
	...
	{
		return Regex.Replace(value ?? string.Empty, "<.*?>", string.Empty, RegexOptions.Singleline);
	}
	...
	private async void TitleBarLanguageToggleButton_Click(object sender, RoutedEventArgs e)
	{
		e.Handled = true;
		...
		await RefreshConfirmationsAfterLanguageChangeAsync();
	}

	private async Task RefreshConfirmationsAfterLanguageChangeAsync()
	{
		if (ConfirmationsSection.Visibility == Visibility.Visible)
		{
			await ShowConfirmationsSectionAsync(forceRefresh: true);
		}
	}
	...
	private void UpdateTitleBarLanguageToggleText()
	{
		...
		{
			_titleBarLanguageToggleButton.Content = (IsEnglishLanguageSelected() ? "PT-BR" : "EN");
		}
	}
	...
	{
		ApplyControlAccountsSidebarLayout();
		ApplyControlAccountsJoinedChrome();
		base.Dispatcher.BeginInvoke(new Action(ApplyControlAccountsJoinedChrome), DispatcherPriority.Loaded);
		base.Dispatcher.BeginInvoke(new Action(RemoveSidebarFooter), DispatcherPriority.Loaded);
		base.Dispatcher.BeginInvoke(new Action(RemoveControlAccountsExtraLines), DispatcherPriority.Loaded);
	}

	private void ApplyControlAccountsSidebarLayout()
	{
		ApplyControlAccountsShellDimensions();
		System.Windows.Controls.Button[] sidebarButtons = new System.Windows.Controls.Button[4] { SidebarHomeButton, SidebarAccountsButton, SidebarConfirmationsButton, SidebarSettingsButton };
		ControlTemplate sidebarButtonTemplate = CreateSidebarButtonTemplate();
		foreach (System.Windows.Controls.Button button in sidebarButtons)
		{
			button.Template = sidebarButtonTemplate;
			button.Height = 44.0;
			button.MinHeight = 44.0;
			button.Width = 196.0;
			button.Margin = new Thickness(0.0, 0.0, 0.0, 10.0);
			button.Padding = new Thickness(18.0, 0.0, 0.0, 0.0);
			...
			ScaleImages(button, 24.0);
			foreach (TextBlock textBlock in EnumerateVisualTree(button).OfType<TextBlock>())
			{
				textBlock.FontSize = 15.0;
				textBlock.VerticalAlignment = VerticalAlignment.Center;
				textBlock.TextTrimming = TextTrimming.CharacterEllipsis;
				textBlock.MaxWidth = 138.0;
			}
		}
		foreach (DependencyObject item in EnumerateVisualTree(this))
		{
			...
			{
				string text = (textBlock.Text ?? string.Empty).Trim();
				if (string.Equals(text, "Renanzinho", StringComparison.OrdinalIgnoreCase))
				{
					HideAncestorButtonOrSelf(textBlock);
				}
				else if (string.Equals(text, "Navegacao", StringComparison.OrdinalIgnoreCase) || string.Equals(text, "Navegação", StringComparison.OrdinalIgnoreCase) || string.Equals(text, "Navigation", StringComparison.OrdinalIgnoreCase))
				{
					textBlock.Visibility = Visibility.Collapsed;
				}
				else if (text.StartsWith("Cofre portatil", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Portable vault", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Cofre portátil", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Cofre desktop", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Desktop vault", StringComparison.OrdinalIgnoreCase))
				{
					textBlock.Visibility = Visibility.Collapsed;
				}
			}
			else if (item is ContentControl contentControl && contentControl.Content is string content && string.Equals(content.Trim(), "Renanzinho", StringComparison.OrdinalIgnoreCase))
			{
				HideAncestorButtonOrSelf(contentControl);
			}
			else if (item is System.Windows.Controls.Image image)
			{
				string source = image.Source?.ToString()?.ToLowerInvariant() ?? string.Empty;
				if (source.Contains("rexpeita-avatar") || source.Contains("rexpeita-skull"))
				{
					image.Width = 108.0;
					image.Height = 108.0;
					image.Margin = new Thickness(0.0, 0.0, 8.0, 22.0);
					image.Stretch = Stretch.Uniform;
					Border? logoBorder = FindAncestor<Border>(image);
					if (logoBorder != null)
					{
						logoBorder.Background = System.Windows.Media.Brushes.Transparent;
						logoBorder.BorderBrush = System.Windows.Media.Brushes.Transparent;
						logoBorder.BorderThickness = new Thickness(0.0);
						logoBorder.MinHeight = 130.0;
						logoBorder.Margin = new Thickness(0.0, 0.0, 0.0, 0.0);
						logoBorder.Padding = new Thickness(0.0);
					}
				}
			}
		}
		RemoveSidebarFooter();
		ApplyCurrentSidebarSelection();
	}

	private void ApplyCurrentSidebarSelection()
	{
	...
	private static ControlTemplate CreateSidebarButtonTemplate()
	{
		...
		return new ControlTemplate(typeof(System.Windows.Controls.Button))
		{
			VisualTree = border
		};
	}

	private void BuildControlAccountsSidebar(Grid sidebarGrid, IReadOnlyList<System.Windows.Controls.Button> sidebarButtons)
	{
		if (sidebarGrid.ColumnDefinitions.Count > 0)
		{
			RemoveSidebarFooter();
			return;
		}
		foreach (System.Windows.Controls.Button button in sidebarButtons)
		{
			if (button.Parent is System.Windows.Controls.Panel parent)
			{
				parent.Children.Remove(button);
			}
		}
		sidebarGrid.Children.Clear();
		sidebarGrid.Background = CreateControlAccountsSidebarBackground();
		StackPanel navigation = new StackPanel
		{
			Margin = new Thickness(24.0, 24.0, 16.0, 0.0),
			VerticalAlignment = VerticalAlignment.Top
		};
		System.Windows.Controls.Image logo = new System.Windows.Controls.Image
		{
			Source = new BitmapImage(new Uri("pack://application:,,,/assets/rexpeita-avatar.png", UriKind.Absolute)),
			Width = 108.0,
			Height = 108.0,
			Stretch = Stretch.Uniform,
			HorizontalAlignment = System.Windows.HorizontalAlignment.Center,
			Margin = new Thickness(0.0, 0.0, 8.0, 22.0)
		};
		RenderOptions.SetBitmapScalingMode(logo, BitmapScalingMode.HighQuality);
		navigation.Children.Add(logo);
		foreach (System.Windows.Controls.Button button in sidebarButtons)
		{
	...
	private static System.Windows.Media.Brush CreateControlAccountsSidebarBackground()
	{
		RadialGradientBrush brush = new RadialGradientBrush
		{
			Center = new System.Windows.Point(0.5, 0.13),
			GradientOrigin = new System.Windows.Point(0.5, 0.13),
			RadiusX = 0.78,
			RadiusY = 0.48
		};
		brush.GradientStops.Add(new GradientStop(System.Windows.Media.Color.FromRgb(13, 43, 66), 0.0));
		brush.GradientStops.Add(new GradientStop(System.Windows.Media.Color.FromRgb(6, 17, 29), 0.72));
		brush.GradientStops.Add(new GradientStop(System.Windows.Media.Color.FromRgb(5, 12, 21), 1.0));
		return brush;
	}

	private void ApplyControlAccountsShellDimensions()
	{
		base.Width = 1260.0;
		base.Height = 700.0;
		base.MinWidth = 1060.0;
		base.MinHeight = 620.0;
		System.Windows.Media.Brush sidebarSurfaceBrush = (System.Windows.Media.Brush)FindResource("ShellSurfaceBrush");
		DependencyObject? current = SidebarHomeButton;
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
			if (current is Grid grid)
			{
				grid.Background = sidebarSurfaceBrush;
				foreach (ColumnDefinition column in grid.ColumnDefinitions)
				{
					if (column.ActualWidth > 180.0 && column.ActualWidth < 280.0)
					{
						column.Width = new GridLength(256.0);
						break;
					}
				}
			}
			if (current is Border border && border.ActualWidth > 180.0 && border.ActualWidth < 280.0)
			{
				border.Width = 256.0;
				border.Background = sidebarSurfaceBrush;
				border.BorderBrush = (System.Windows.Media.Brush)FindResource("BorderBrush");
				border.BorderThickness = new Thickness(0.0);
			}
			if (current is System.Windows.Controls.Panel panel && panel.ActualWidth > 180.0 && panel.ActualWidth < 280.0)
			{
				panel.Background = sidebarSurfaceBrush;
			}
		}
	}

	private void AddSidebarFooter()
	{
		RemoveSidebarFooter();
		HideOriginalSidebarBottomPill();
	}

	private void RemoveSidebarFooter()
	{
		if (!(SidebarSettingsButton.Parent is System.Windows.Controls.Panel navigationPanel))
		{
			return;
		}
		System.Windows.Controls.Panel footerHost = FindSidebarFooterHost(navigationPanel) ?? navigationPanel;
		foreach (FrameworkElement oldFooter in footerHost.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsSidebarFooter", StringComparison.Ordinal)).ToList())
		{
			footerHost.Children.Remove(oldFooter);
		}
	}

	private void HideOriginalSidebarBottomPill()
	{
		foreach (FrameworkElement element in EnumerateVisualTree(this).OfType<FrameworkElement>())
		{
			if (string.Equals(element.Tag?.ToString(), "ControlAccountsSidebarFooter", StringComparison.Ordinal))
			{
				continue;
			}
			if (element is not Border && element is not System.Windows.Controls.Button)
			{
				continue;
			}
			double width = element.ActualWidth > 0.0 ? element.ActualWidth : element.Width;
			double height = element.ActualHeight > 0.0 ? element.ActualHeight : element.Height;
			if (width < 150.0 || width > 230.0 || height < 16.0 || height > 42.0)
			{
				continue;
			}
			...
			try
			{
				point = element.TranslatePoint(new System.Windows.Point(0.0, 0.0), this);
			}
			catch
			{
				continue;
			}
			if (point.X < 10.0 || point.X > 250.0 || point.Y < base.ActualHeight - 90.0)
			{
				continue;
			}
	...
	private void RemoveControlAccountsExtraLines()
	{
		foreach (Border border in EnumerateVisualTree(this).OfType<Border>())
		{
			if (border.Tag?.ToString()?.StartsWith("ControlAccounts", StringComparison.Ordinal) == true)
			{
				continue;
			}
			if (border.ActualWidth < 320.0)
			{
				continue;
			}
			System.Windows.Point point;
			try
			{
				point = border.TranslatePoint(new System.Windows.Point(0.0, 0.0), this);
			}
			catch
			{
				continue;
			}
			bool isTopChromeBorder = point.Y >= -1.0 && point.Y <= 72.0 && border.ActualHeight <= 80.0;
			bool isThinHorizontalLine = border.ActualHeight <= 3.0;
			if (isTopChromeBorder || isThinHorizontalLine)
			{
				border.BorderThickness = new Thickness(0.0);
				if (isThinHorizontalLine)
				{
	...
	private void ApplyControlAccountsJoinedChrome()
	{
		RemoveOuterInsetMargin();
		Grid? titleRootGrid = FindTitleRootGrid();
		if (titleRootGrid != null && titleRootGrid.RowDefinitions.Count > 1)
		{
			titleRootGrid.RowDefinitions[0].Height = new GridLength(32.0);
			RemoveTitleSidebarDivider(titleRootGrid);
			AddTitleHorizontalDivider(titleRootGrid);
			RemoveBodySidebarDivider(titleRootGrid);
			RemoveNativeSidebarDivider();
			RemoveFooterDivider(titleRootGrid);
			HideOriginalBottomStatusBar();
			ReserveSpaceForFullWidthFooter();
			RemoveTitleIconFrame();
			AddFullWidthBottomFooter(titleRootGrid);
		}
		Border? titleBorder = FindTitleBarBorder();
		if (titleBorder != null)
		{
			titleBorder.Height = 32.0;
			titleBorder.MinHeight = 32.0;
			titleBorder.Padding = new Thickness(0.0);
			titleBorder.Margin = new Thickness(0.0);
			titleBorder.Background = new SolidColorBrush(System.Windows.Media.Color.FromRgb(8, 23, 36));
			titleBorder.BorderThickness = new Thickness(0.0, 0.0, 0.0, 1.0);
			titleBorder.BorderBrush = (System.Windows.Media.Brush)FindResource("BorderBrush");
		}
		FrameworkElement? shell = FindOuterShell();
		if (shell != null)
		{
			shell.Margin = new Thickness(0.0);
			if (shell is Border shellBorder)
			{
				shellBorder.CornerRadius = new CornerRadius(0.0);
				shellBorder.Padding = new Thickness(0.0);
				shellBorder.BorderThickness = new Thickness(0.0);
			}
		}
		if (SidebarHomeButton.Parent is FrameworkElement navigationHost)
		{
			navigationHost.Margin = new Thickness(8.0, 34.0, 14.0, 0.0);
		}
	}

	private void AddTitleSidebarDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsTitleDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
		}
		Border divider = new Border
		{
			Tag = "ControlAccountsTitleDivider",
			Width = 1.0,
			HorizontalAlignment = System.Windows.HorizontalAlignment.Left,
			VerticalAlignment = VerticalAlignment.Stretch,
			Margin = new Thickness(256.0, 0.0, 0.0, 0.0),
	...
	private void RemoveTitleSidebarDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsTitleDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
	...
	private void AddTitleHorizontalDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsTitleHorizontalDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
		}
		Border divider = new Border
		{
			Tag = "ControlAccountsTitleHorizontalDivider",
			Height = 1.0,
		...
		{
			Grid.SetColumnSpan(divider, rootGrid.ColumnDefinitions.Count);
		}
		rootGrid.Children.Add(divider);
	}

	private void AddBodySidebarDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsBodySidebarDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
		}
		Border divider = new Border
		{
			Tag = "ControlAccountsBodySidebarDivider",
			Width = 1.0,
			HorizontalAlignment = System.Windows.HorizontalAlignment.Left,
			VerticalAlignment = VerticalAlignment.Stretch,
			Margin = new Thickness(257.0, 0.0, 0.0, 0.0),
			Background = (System.Windows.Media.Brush)FindResource("BorderBrush")
		};
	...
	private void RemoveBodySidebarDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsBodySidebarDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
	...
	private void RemoveNativeSidebarDivider()
	{
		foreach (Border border in EnumerateVisualTree(this).OfType<Border>())
		{
			if (border.Tag?.ToString()?.StartsWith("ControlAccounts", StringComparison.Ordinal) == true)
			...
			double width = border.ActualWidth > 0.0 ? border.ActualWidth : border.Width;
			double height = border.ActualHeight > 0.0 ? border.ActualHeight : border.Height;
			if (width > 3.0 || height < 220.0)
			{
				continue;
			}
			System.Windows.Point point;
			...
			if (point.X < 230.0 || point.X > 290.0)
			{
				continue;
			}
			border.Visibility = Visibility.Collapsed;
	...
	private void RemoveTitleIconFrame()
	{
		foreach (Border border in EnumerateVisualTree(this).OfType<Border>())
		{
			if (border.Tag?.ToString()?.StartsWith("ControlAccounts", StringComparison.Ordinal) == true)
			...
			double width = border.ActualWidth > 0.0 ? border.ActualWidth : border.Width;
			double height = border.ActualHeight > 0.0 ? border.ActualHeight : border.Height;
			if (width < 18.0 || width > 36.0 || height < 18.0 || height > 36.0)
			{
				continue;
			}
			System.Windows.Point point;
			...
			if (point.X > 52.0 || point.Y > 40.0)
			{
				continue;
			}
			bool containsIcon = EnumerateVisualTree(border).OfType<System.Windows.Controls.Image>().Any() || EnumerateVisualTree(border).OfType<System.Windows.Shapes.Rectangle>().Any() || EnumerateVisualTree(border).OfType<TextBlock>().Any();
			if (!containsIcon)
			{
				continue;
			}
	...
	private void AddFooterDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsFooterDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
		}
		Border divider = new Border
		{
			Tag = "ControlAccountsFooterDivider",
			Height = 1.0,
			HorizontalAlignment = System.Windows.HorizontalAlignment.Stretch,
			VerticalAlignment = VerticalAlignment.Bottom,
	...
	private void RemoveFooterDivider(Grid rootGrid)
	{
		foreach (FrameworkElement oldDivider in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsFooterDivider", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldDivider);
	...
	private void AddFullWidthBottomFooter(Grid rootGrid)
	{
		foreach (FrameworkElement oldFooter in rootGrid.Children.OfType<FrameworkElement>().Where((FrameworkElement x) => string.Equals(x.Tag?.ToString(), "ControlAccountsFullWidthBottomFooter", StringComparison.Ordinal)).ToList())
		{
			rootGrid.Children.Remove(oldFooter);
		}
		Border footer = new Border
		{
			Tag = "ControlAccountsFullWidthBottomFooter",
			Height = 38.0,
			HorizontalAlignment = System.Windows.HorizontalAlignment.Stretch,
			VerticalAlignment = VerticalAlignment.Bottom,
		...
		row.Children.Add(new TextBlock
		{
			Text = L("Sistema protegido - \u00faltima atualiza\u00e7\u00e3o: hoje, ", "Protected system - last update: today, ") + DateTime.Now.ToString("HH:mm"),
		...
		{
			Grid.SetRowSpan(footer, rootGrid.RowDefinitions.Count);
		}
		...
		System.Windows.Controls.Panel.SetZIndex(footer, 1000);
		rootGrid.Children.Add(footer);
	}

	private void ReserveSpaceForFullWidthFooter()
	{
		Thickness sectionPadding = new Thickness(0.0, 0.0, 0.0, 54.0);
		foreach (ScrollViewer section in new[] { HomeSection, NewAccountSection, ConfirmationsSection, SettingsSection })
		{
			if (section == null)
			{
				continue;
			...
			section.Margin = new Thickness(section.Margin.Left, section.Margin.Top, section.Margin.Right, 0.0);
		}
		if (AccountsList != null)
		{
			AccountsList.Padding = new Thickness(0.0, 0.0, 0.0, 44.0);
		}
		if (ConfirmationsList != null)
		{
			ConfirmationsList.Padding = new Thickness(0.0, 0.0, 0.0, 44.0);
		}
	}

	private void HideOriginalBottomStatusBar()
	{
		double windowHeight = base.ActualHeight > 0.0 ? base.ActualHeight : base.Height;
		foreach (TextBlock textBlock in EnumerateVisualTree(this).OfType<TextBlock>())
		{
			if (textBlock.Tag?.ToString()?.StartsWith("ControlAccounts", StringComparison.Ordinal) == true)
			{
				continue;
			}
			string text = (textBlock.Text ?? string.Empty).Trim();
			...
			bool looksLikeBottomStatus = string.Equals(text, ViewModel.AppVersion, StringComparison.OrdinalIgnoreCase) || text.StartsWith("Cofre ", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Vault ", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Idioma ", StringComparison.OrdinalIgnoreCase) || text.StartsWith("Language ", StringComparison.OrdinalIgnoreCase);
			if (!looksLikeBottomStatus)
			{
				continue;
			}
			System.Windows.Point point;
			try
			{
				point = textBlock.TranslatePoint(new System.Windows.Point(0.0, 0.0), this);
			}
			catch
			{
				continue;
			}
			if (point.Y < windowHeight - 82.0)
			{
				continue;
			}
			FrameworkElement? statusHost = FindCompactBottomHost(textBlock);
			if (statusHost != null)
			{
	...
	private FrameworkElement? FindCompactBottomHost(FrameworkElement start)
	{
		FrameworkElement? best = start;
		...
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
			...
			if (element.Tag?.ToString()?.StartsWith("ControlAccounts", StringComparison.Ordinal) == true)
			{
				break;
			}
			double height = element.ActualHeight > 0.0 ? element.ActualHeight : element.Height;
			if (height > 0.0 && height <= 58.0)
			{
				best = element;
				continue;
			}
	...
	private void RemoveOuterInsetMargin()
	{
		foreach (Grid grid in EnumerateVisualTree(this).OfType<Grid>())
		{
			if (grid.ActualWidth > base.ActualWidth - 80.0 && grid.ActualHeight > base.ActualHeight - 80.0 && grid.Margin.Left >= 15.0 && grid.Margin.Top >= 15.0)
			{
		...
		foreach (Border border in EnumerateVisualTree(this).OfType<Border>())
		{
			if (border.ActualWidth > base.ActualWidth - 80.0 && border.ActualHeight > base.ActualHeight - 80.0)
			{
				border.CornerRadius = new CornerRadius(0.0);
				border.BorderThickness = new Thickness(0.0);
			}
	...
	private Grid? FindTitleRootGrid()
	{
		DependencyObject? current = MinimizeButton;
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
			if (current is Grid grid && grid.RowDefinitions.Count > 1)
			{
				return grid;
			}
		}
	...
	private Border? FindTitleBarBorder()
	{
		DependencyObject? current = MinimizeButton;
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
			if (current is Border border && border.ActualWidth > 300.0 && border.ActualHeight > 20.0 && border.ActualHeight < 80.0)
			{
				return border;
			}
		}
	...
	private FrameworkElement? FindOuterShell()
	{
		return EnumerateVisualTree(this).OfType<Border>().Where((Border border) => border.ActualWidth > base.ActualWidth - 80.0 && border.ActualHeight > base.ActualHeight - 80.0).OrderByDescending((Border border) => border.ActualWidth * border.ActualHeight).FirstOrDefault();
	}

	private static Grid? FindSidebarContentGrid(DependencyObject start)
	{
		DependencyObject? current = start;
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
			if (current is Grid grid && grid.ColumnDefinitions.Count == 0 && VisualTreeHelper.GetParent(grid) is Border)
			{
				return grid;
			}
			if (current is Border border && border.Child is Grid childGrid && IsSidebarWidth(border))
			{
				return childGrid;
			}
		}
	...
	private static bool IsSidebarWidth(FrameworkElement element)
	{
		double width = element.ActualWidth > 0.0 ? element.ActualWidth : element.Width;
		return width > 180.0 && width < 280.0;
	}

	private Border CreateControlAccountsSidebarFooter()
	{
		Border footer = new Border
		{
			Tag = "ControlAccountsSidebarFooter",
			Width = 196.0,
			Margin = new Thickness(24.0, 0.0, 0.0, 22.0),
			HorizontalAlignment = System.Windows.HorizontalAlignment.Left,
			VerticalAlignment = VerticalAlignment.Bottom,
			BorderThickness = new Thickness(0.0),
		...
		TextBlock icon = new TextBlock
		{
			Text = "\uE83D",
			FontFamily = new System.Windows.Media.FontFamily("Segoe MDL2 Assets"),
			Foreground = new SolidColorBrush(System.Windows.Media.Color.FromRgb(38, 185, 108)),
			FontSize = 16.0,
			Width = 26.0,
		...
		TextBlock version = new TextBlock
		{
			...
			Foreground = (System.Windows.Media.Brush)FindResource("MutedBrush"),
			Margin = new Thickness(40.0, 3.0, 0.0, 0.0),
			FontSize = 12.0
		};
	...
	private static void HideAncestorButtonOrSelf(FrameworkElement element)
	{
		System.Windows.Controls.Button? button = FindAncestor<System.Windows.Controls.Button>(element);
		if (button != null)
		{
	...
	private static System.Windows.Controls.Panel? FindSidebarFooterHost(DependencyObject start)
	{
		DependencyObject? current = start;
		while ((current = VisualTreeHelper.GetParent(current)) != null)
		{
	...
	private void UpdateSidebarFooterText(DependencyObject footer)
	{
		List<TextBlock> textBlocks = EnumerateVisualTree(footer).OfType<TextBlock>().ToList();
		if (textBlocks.Count > 0)
		{
			textBlocks[0].Text = L("Sistema protegido", "Protected system");
		}
		if (textBlocks.Count > 1)
		{
			textBlocks[1].Text = ViewModel.AppVersion;
		}
	}

	private static void ScaleImages(DependencyObject root, double size)
	{
		foreach (DependencyObject item in EnumerateVisualTree(root))
		{
	...
	private void ApplyDarkTheme()
	{
		...
		SetBrushColor("SidebarBrush", 3, 21, 39);
	...
	private async void LanguageSettingsComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
	{
		if (base.IsLoaded && !_updatingSettingsUi)
		{
